...
void DataScope_Send_3axis_Data(float x_val, float y_val, float z_val)
{
	uint8_t len;
	DataScope_Get_Channel_Data(x_val, 1);
	DataScope_Get_Channel_Data(y_val, 2);
	DataScope_Get_Channel_Data(z_val, 3);
	len = DataScope_Data_Generate(3);
	tx_com(DataScope_OutPut_Buffer,len);
}
